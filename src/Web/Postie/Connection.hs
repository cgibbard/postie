...
mkSocketConnection sock = do
    conn <- newIORef (ConnPlain sock)
    return (Connection conn)

connBackendRecv :: ConnectionBackend -> IO BS.ByteString
connBackendRecv (ConnPlain sock) = recv sock defaultChunkSize
connBackendRecv (ConnSecure ctx)   = recvData ctx

connBackendSend :: ConnectionBackend -> LBS.ByteString -> IO ()
connBackendSend (ConnPlain sock) = sendAll sock
...
connClose (Connection cbe) = closeBackend =<< readIORef cbe
  where
    closeBackend (ConnPlain sock)   = sClose sock
